...
def oneHotEncode(classes: int, y: np.ndarray) -> np.ndarray:
    ...
    oneHotEncoded = np.eye(classes, dtype=np.int8)[y]
...
def quantize(x: np.ndarray, precision=32):
    ...
    np.multiply(x, intervals, out=x)
    np.round_(x, out=x)
    np.divide(x, intervals, out=x)
...
def loadIdxFile(file_pth: Path, isTest: bool) -> np.ndarray:
    """
...
def loadNpyFile(file_pth: Path, isLabelFile: bool) -> np.ndarray:
    """
    ...
    data = np.load(file_pth).astype(dt)

    return data


def loadX(
    file_pth: Path, loadFunc, num_instances: int, num_features: int
) -> np.ndarray:
    """
...
def loadY(file_pth: Path, loadFunc, encoded=True) -> np.ndarray:
    """
    ...
    y = loadFunc(file_pth, True).astype(np.int8)
...
def loadIdxTraining(dataDir: Path, encoded=True) -> XY_DATA:
    """
    ...
    X = loadX(xPth, loadIdxFile, instances, features)
    y = loadY(yPth, loadIdxFile, encoded=encoded)

    return X, y


def loadIdxTesting(dataDir: Path) -> XY_DATA:
    """
    ...
    X = loadX(xPth, loadIdxFile, instances, features)
    y = loadY(yPth, loadIdxFile, encoded=False).astype(np.uint8)
...
def loadNpyTraining(
    dataDir: Path, instances: int, features: int, encoded=True
) -> XY_DATA:
    """
    ...
    X = loadX(xPth, loadNpyFile, instances, features)
    y = loadY(yPth, loadNpyFile, encoded=encoded)

    return X, y


def loadNpyTesting(dataDir: Path, instances: int, features: int) -> XY_DATA:
    """
    ...
    X = loadX(xPth, loadNpyFile, instances, features)
    y = loadY(yPth, loadNpyFile, encoded=False).astype(np.uint8)
...
def loadMnistC(category: DATASETS, encoded=True) -> TRAIN_TEST_DATA:
    """
    Loads MNIST-C data
    ...
    trainX, trainY = loadNpyTraining(dirPth, trainInstances, features, encoded=encoded)
    testX, testY = loadNpyTesting(dirPth, testInstances, features)
...
def loadFashionMnist(encoded=True) -> TRAIN_TEST_DATA:
    """
    ...
    trainX, trainY = loadIdxTraining(FASHIONMNISTDIR, encoded=encoded)
    testX, testY = loadIdxTesting(FASHIONMNISTDIR)

    return trainX, trainY, testX, testY


def loadAffNist(encoded=True) -> TRAIN_TEST_DATA:
    ...
    trainX = loadX(
        root / f"{prefix}_trainX.npy", loadNpyFile, train_instances, features,
    )
    trainY = loadY(root / f"{prefix}_trainY.npy", loadNpyFile, encoded=encoded)
    testX = loadX(
        root / f"{prefix}_testX.npy", loadNpyFile, test_validation_instances, features,
    )
    testY = loadY(root / f"{prefix}_testY.npy", loadNpyFile, encoded=False).astype(
        np.uint8
    )
...
def loadCifar10(encoded=True) -> TRAIN_TEST_DATA:
    """
...
def loadDataset(
    ...
    useGpu
        Only to provide compatibility with utils. Doesn't affect anything
    ...
    if str(dataset).startswith("mnist_c"):
        data = loadMnistC(dataset, encoded=encoded)
    else:
        loadFunc = LOADING_FUNCS[dataset]
        data = loadFunc(encoded=encoded)
